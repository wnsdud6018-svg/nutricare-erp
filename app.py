...
# 세션 기본값 (주간 식단 및 영양 데이터)
if "weekly_menu" not in st.session_state:
    st.session_state["weekly_menu"] = pd.DataFrame([
        {"구분": "월요일", "아침": "쌀밥 / 콩나물국 / 계란찜 / 무생채", "점심": "잡곡밥 / 돈육김치찌개 / 가자미구이 / 시금치나물", "저녁": "쌀밥 / 아욱된장국 / 마파두부 / 깍두기", "간식": "두유 / 바나나", "열량(kcal)": 1650, "단백질(g)": 68, "지방(g)": 38, "나트륨(mg)": 1850},
        {"구분": "화요일", "아침": "야채죽 / 미역국 / 두부조림 / 겉절이", "점심": "쌀밥 / 소고기무국 / 제육볶음 / 콩나물무침", "저녁": "잡곡밥 / 순두부찌개 / 계란말이 / 열무김치", "간식": "찐고구마 / 우유", "열량(kcal)": 1720, "단백질(g)": 72, "지방(g)": 42, "나트륨(mg)": 1920},
        {"구분": "수요일", "아침": "쌀밥 / 북엇국 / 감자채볶음 / 포기김치", "점심": "카레라이스 / 유부장국 / 닭강정 / 단무지무침", "저녁": "쌀밥 / 동태찌개 / 떡갈비조림 / 나물무침", "간식": "카스테라 / 요플레", "열량(kcal)": 1680, "단백질(g)": 65, "지방(g)": 40, "나트륨(mg)": 1890},
        {"구분": "목요일", "아침": "잣죽 / 된장찌개 / 어묵볶음 / 깍두기", "점심": "잡곡밥 / 갈비탕 / 오징어볶음 / 취나물무침", "저녁": "쌀밥 / 콩가루배추국 / 제육간장조림 / 김치", "간식": "제철과일 / 오렌지주스", "열량(kcal)": 1750, "단백질(g)": 74, "지방(g)": 44, "나트륨(mg)": 1980},
        {"구분": "금요일", "아침": "쌀밥 / 시래깃국 / 호박전 / 포기김치", "점심": "비빔밥 / 계란파국 / 새우튀김 / 백김치", "저녁": "잡곡밥 / 부대찌개 / 삼치구이 / 숙주나물", "간식": "찐옥수수 / 둥굴레차", "열량(kcal)": 1690, "단백질(g)": 70, "지방(g)": 39, "나트륨(mg)": 1840},
        {"구분": "토요일", "아침": "소고기죽 / 계란국 / 연두부 / 겉절이", "점심": "쌀밥 / 청국장찌개 / 안동찜닭 / 무말랭이", "저녁": "쌀밥 / 오징어무국 / 동그랑땡 / 포기김치", "간식": "단호박죽", "열량(kcal)": 1630, "단백질(g)": 66, "지방(g)": 36, "나트륨(mg)": 1790},
        {"구분": "일요일", "아침": "쌀밥 / 팽이버섯국 / 스크램블 / 김치", "점심": "짜장밥 / 계란부용국 / 탕수육 / 포기김치", "저녁": "잡곡밥 / 육개장 / 생선전 / 청포묵무침", "간식": "떡 / 식혜", "열량(kcal)": 1710, "단백질(g)": 69, "지방(g)": 41, "나트륨(mg)": 1910}
    ])
...
if "logged_in" not in st.session_state or not st.session_state["logged_in"]:
    ...
    # ---------------------------------------------------------
    # [메뉴 6] 주간 식단표 관리 & 자동 영양판정 모듈 (신규 탑재)
    # ---------------------------------------------------------
    elif menu == "6. 주간 식단표 관리 (엑셀 연동 & 영양판정)":
        st.title("📅 주간 식단표 관리 및 실시간 영양 적정성 자동 판정")
        st.caption("건보공단 요양원 영양 기준(1일 1,600~1,800kcal / 단백질 60g 이상)을 기반으로 자동 평가합니다.")
        st.markdown("---")
        ...
        st.markdown("---")
        st.subheader("📊 주간 식단 영양소 적정성 평가 리포트 (건보공단 감사 대비)")

        # 주간 평균 영양소 자동 계산
        avg_cal = edited_menu["열량(kcal)"].mean()
        avg_prot = edited_menu["단백질(g)"].mean()
        avg_fat = edited_menu["지방(g)"].mean()
        ...
        if avg_prot >= 60:
            ...
            col_n2.metric("1일 평균 단백질", f"{avg_prot:.1f} g", "🔴 부족 (어육류/계란 추가)", delta_color="inverse")

        # 3. 지방 판정
        col_n3.metric("1일 평균 지방", f"{avg_fat:.1f} g", "🟢 적정 수준 (총열량 20%)")

        # 4. 나트륨 판정
        if avg_sod <= 2000:
            col_n4.metric("1일 평균 나트륨", f"{avg_sod:.0f} mg", "🟢 적정 (2,000mg 이하 저염식)")
        else:
            col_n4.metric("1일 평균 나트륨", f"{avg_sod:.0f} mg", "🔴 나트륨 초과 (염도 조절)", delta_color="inverse")

        st.markdown("---")
        st.subheader("💡 20년차 영양사 AI 수석 참모의 종합 피드백")
        
        if avg_prot < 60:
            st.error("⚠️ [영양 개선 권고사항] 주간 평균 단백질 공급량이 어르신 권장량(60g) 미만입니다. 수요일/토요일 간식으로 두유, 두부, 또는 계란찜 비중을 늘리는 것을 추천합니다.")
        else:
            st.success("✅ [영양 기준 완벽 충족] 본 주간 식단표는 노인 영양섭취기준(KDRI) 및 건보공단 요양기관 평가 기준을 100% 충족하는 우수한 식단입니다.")
